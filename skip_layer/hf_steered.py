...
def steer(
    ...
    def create_hook(hookpoint: str):
        def hook_fn(module: nn.Module, input: Any, output: Tensor):
            steer_fn = hook_to_steer[hookpoint]

            if getattr(steer_fn, "_skip_layer", False):
                if isinstance(output, tuple):
                    return (input[0], *output[1:])
                return input[0]

            if isinstance(output, tuple):
                output = (steer_fn(output[0]), *output[1:])
            else:
                output = steer_fn(output)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for hookpoint, steer_info in steer_config.items():
            ...
            steering_vector = steer_info.get("steering_vector", None)

            if steering_vector is not None:
                steering_vector = (
                    steering_vector.to(self.device).to(self.model.dtype)
                )
            ...
            if action == "add":
                ...
                fn = partial(self.skip_layer)
                fn._skip_layer = True
                hook_to_steer[hookpoint] = fn
            else:
